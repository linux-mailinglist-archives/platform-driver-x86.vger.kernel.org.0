...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 0D5351392B0
	for <lists+platform-driver-x86@lfdr.de>; Mon, 13 Jan 2020 14:58:13 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1728964AbgAMN5R (ORCPT
        <rfc822;lists+platform-driver-x86@lfdr.de>);
        Mon, 13 Jan 2020 08:57:17 -0500
Received: from mga11.intel.com ([192.55.52.93]:54643 "EHLO mga11.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1728940AbgAMN4j (ORCPT
...
Subject: [PATCH v3 25/36] platform/x86: intel_pmc_ipc: Start using SCU IPC
Date:   Mon, 13 Jan 2020 16:56:12 +0300
Message-Id: <20200113135623.56286-26-mika.westerberg@linux.intel.com>
...
---
 ...
 drivers/platform/x86/intel_pmc_ipc.c | 297 ++++-----------------------
 2 files changed, 46 insertions(+), 252 deletions(-)

diff --git a/drivers/platform/x86/Kconfig b/drivers/platform/x86/Kconfig
index 50deff627b46..3537dbf45592 100644
...
index 8527327d88c7..bcdff20a304c 100644
...
-
  ...
  * @cmd:	IPC command code.
@@ -411,34 +210,22 @@ static int intel_pmc_ipc_raw_cmd(u32 cmd, u32 sub, u8 *in, u32 inlen, u32 *out,
...
@@ -449,24 +236,25 @@ static int ipc_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
 	if (ret)
 		return ret;
...
@@ -479,12 +267,14 @@ static struct pci_driver ipc_pci_driver = {
 	.name = "intel_pmc_ipc",
...
+	struct intel_scu_ipc_dev *scu = dev_get_drvdata(dev);
 	...
 	int cmd;
 	int ret;
@@ -495,7 +285,7 @@ static ssize_t intel_pmc_ipc_simple_cmd_store(struct device *dev,
 		return -EINVAL;
 	}
...
+	ret = intel_scu_ipc_dev_simple_command(scu, cmd, subcmd);
 	if (ret) {
 		...
 		return ret;
@@ -507,6 +297,7 @@ static ssize_t intel_pmc_ipc_northpeak_store(struct device *dev,
 					     struct device_attribute *attr,
 					     const char *buf, size_t count)
 {
+	struct intel_scu_ipc_dev *scu = dev_get_drvdata(dev);
 	...
 	int subcmd;
 	int ret;
@@ -518,7 +309,7 @@ static ssize_t intel_pmc_ipc_northpeak_store(struct device *dev,
 		subcmd = 1;
 	else
 		subcmd = 0;
...
+	ret = intel_scu_ipc_dev_simple_command(scu, PMC_IPC_NORTHPEAK_CTRL, subcmd);
 	if (ret) {
 		...
 		return ret;
@@ -711,7 +502,8 @@ static int ipc_create_pmc_devices(void)
 	return ret;
 }
...
+			    struct intel_scu_ipc_pdata *pdata)
 {
 	...
 	void __iomem *addr;
@@ -795,7 +587,7 @@ static int ipc_plat_get_res(struct platform_device *pdev)
 	if (IS_ERR(addr))
 		return PTR_ERR(addr);
...
@@ -851,49 +643,50 @@ MODULE_DEVICE_TABLE(acpi, ipc_acpi_ids);
...
@@ -904,10 +697,10 @@ static int ipc_plat_probe(struct platform_device *pdev)
