...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 210423F29A6
	for <lists+platform-driver-x86@lfdr.de>; Fri, 20 Aug 2021 11:57:45 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S237516AbhHTJ6V (ORCPT
        <rfc822;lists+platform-driver-x86@lfdr.de>);
        Fri, 20 Aug 2021 05:58:21 -0400
Received: from wout5-smtp.messagingengine.com ([64.147.123.21]:45133 "EHLO
        wout5-smtp.messagingengine.com" rhost-flags-OK-OK-OK-OK)
        by vger.kernel.org with ESMTP id S235321AbhHTJ6U (ORCPT
        <rfc822;platform-driver-x86@vger.kernel.org>);
        Fri, 20 Aug 2021 05:58:20 -0400
Received: from compute1.internal (compute1.nyi.internal [10.202.2.41])
        by mailout.west.internal (Postfix) with ESMTP id F02643200A76;
        Fri, 20 Aug 2021 05:57:42 -0400 (EDT)
Received: from mailfrontend1 ([10.202.2.162])
  by compute1.internal (MEProxy); Fri, 20 Aug 2021 05:57:43 -0400
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=
        messagingengine.com; h=cc:content-transfer-encoding:date:from
        :in-reply-to:message-id:mime-version:references:subject:to
        :x-me-proxy:x-me-proxy:x-me-sender:x-me-sender:x-sasl-enc; s=
        fm3; bh=IHCJOd3E/yjt/q4+df2x05aFDWY8XramKmykJdMV0Ro=; b=CYMZBTIZ
        
X-ME-Sender: <xms:lnwfYWaWu9NPi-x56MPfUn4UCAvRE9TcVn77LFivVgz_8sBFymgQwA>
    <xme:lnwfYZbJ_BFRywM1KMR_vDZjxa5FK6eBfI7s8oktb-Zn9c8LM93fV-6L6nZsKs4D9
    wTVa3OW0XWzf6v4UPM>
X-ME-Received: <xmr:lnwfYQ_xZqfQMmcVAiXrWcfmMyZElugQRkihGPRd78-S3mG_xqCYm99-OUU6>
...
X-ME-Proxy: <xmx:lnwfYYqJH0Tdg18ex7U9n1yYqrNhoy8DIl2LRBB2sDVruGJc2D3QnQ>
    <xmx:lnwfYRpQewql_JRwZTf2XrhUNl0cfLQ1Y344R7llqdV4DylhV-ZtEw>
    <xmx:lnwfYWQTVyWq-A5gv1CLTf2q0Ets529hIkkj67V42j_AgejxelZjdg>
    <xmx:lnwfYY2GMmcT_mYsDt09FK7Gq5GGGo_iaRzPo5Ah5YPNT3UTjpW4JA>
Received: by mail.messagingengine.com (Postfix) with ESMTPA; Fri,
 20 Aug 2021 05:57:38 -0400 (EDT)
From:   "Luke D. Jones" <luke@ljones.dev>
To:     linux-kernel@vger.kernel.org
Cc:     hdegoede@redhat.com, hadess@hadess.net,
        platform-driver-x86@vger.kernel.org,
        "Luke D. Jones" <luke@ljones.dev>
Subject: [PATCH v4 1/1] asus-wmi: Add support for custom fan curves
Date:   Fri, 20 Aug 2021 21:57:26 +1200
Message-Id: <20210820095726.14131-2-luke@ljones.dev>
...
+								char *default_curve, u32 throttle_policy);
+
...
@@ -2043,6 +2151,461 @@ static ssize_t fan_boost_mode_store(struct device *dev,
...
+*/
+
+static int active_fan_curve_profiles_check_present(struct asus_wmi *asus)
+{
...
+
+
+static int active_fan_curve_profiles_write(struct asus_wmi *asus, const char *names)
+{
...
+				|| strcmp(set, "balanced\n") == 0)
+			asus->active_fan_curve_profiles.balanced = true;
+
...
+			asus->active_fan_curve_profiles.performance = true;
+	}
+
+	err = throttle_thermal_policy_write(asus);
+	if (err)
...
@@ -2067,6 +2630,59 @@ static int throttle_thermal_policy_check_present(struct asus_wmi *asus)
 	return 0;
 }
...
+
 ...
 {
 	int err;
@@ -2092,6 +2708,26 @@ static int throttle_thermal_policy_write(struct asus_wmi *asus)
 		return -EIO;
 	}
...
@@ -2741,6 +3384,20 @@ static umode_t asus_sysfs_is_visible(struct kobject *kobj,
...
@@ -3016,6 +3673,20 @@ static int asus_wmi_add(struct platform_device *pdev)
 	else
...
@@ -3109,6 +3780,7 @@ static int asus_wmi_add(struct platform_device *pdev)
